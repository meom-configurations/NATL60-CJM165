...
CONTAINS
   ...
   SUBROUTINE iom_init( cdname ) 
      ...
      SELECT CASE ( nleapy )        ! Choose calendar for IOIPSL
      CASE ( 1)   ; CALL xios_define_calendar( TYPE = "Gregorian", time_origin = xios_date(1958,01,01,00,00,00), &
          &                                    start_date = xios_date(nyear,nmonth,nday,0,0,0) )
      CASE ( 0)   ; CALL xios_define_calendar( TYPE = "NoLeap"   , time_origin = xios_date(1958,01,01,00,00,00), &
          &                                    start_date = xios_date(nyear,nmonth,nday,0,0,0) )
      CASE (30)   ; CALL xios_define_calendar( TYPE = "D360"     , time_origin = xios_date(1958,01,01,00,00,00), &
          &                                    start_date = xios_date(nyear,nmonth,nday,0,0,0) )
      END SELECT
#endif
      ...
      IF( TRIM(cdname) == TRIM(cxios_context) ) THEN  
         ...
         CALL set_grid( "F", glamf, gphif ) 
         CALL set_grid( "UW", glamu, gphiu )
         CALL set_grid( "VW", glamv, gphiv )
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE iom_set_zoom_domain_attr
   ...
   SUBROUTINE set_grid( cdgrd, plon, plat )
     ...
     CALL iom_set_domain_attr("grid_"//cdgrd, ni_glo=jpiglo, nj_glo=jpjglo, ibegin=nimpp+nldi-2, jbegin=njmpp+nldj-2, ni=ni, nj=nj)
      ...
      IF ( ln_mskland ) THEN
         ...
         CASE('UW')  ;   zmask(2:jpim1,:,:) = tmask(2:jpim1,:,:) + tmask(3:jpi,:,:)   ; zmask(:,:,2:jpk  ) = tmask(:,:,1:jpkm1) + tmask(:,:,2:jpk)   ;  zmask(:,:,1) = tmask(:,:,1); CALL lbc_lnk( zmask, 'U', 1. )
         CASE('VW')  ;   zmask(:,:,2:jpk  ) = tmask(:,:,1:jpkm1) + tmask(:,:,2:jpk)   ; zmask(:,2:jpjm1,:) = tmask(:,2:jpjm1,:) + tmask(:,3:jpj,:)   ;  zmask(:,:,1) = tmask(:,:,1); CALL lbc_lnk( zmask, 'V', 1. )
         CASE('F')   ;   zmask(2:jpim1,:,:) = tmask(2:jpim1,:,:) + tmask(3:jpi,:,:)   ; zmask(:,2:jpjm1,:) = tmask(:,2:jpjm1,:) + tmask(:,3:jpj,:)   ;  CALL lbc_lnk( zmask, 'F', 1. )
         END SELECT
         !
#if ! defined key_xios2
   ...
   SUBROUTINE set_grid_bounds( cdgrd, plon_cnr, plat_cnr, plon_pnt, plat_pnt )
      ...
      CALL lbc_lnk( z_fld, cl_grd, -1. )    ! Working array for location of northfold
      ...
      DO jn = 1, 4
         CALL lbc_lnk( z_bnds(jn,:,:,1), cl_grd, 1., pval=999._wp )
         CALL lbc_lnk( z_bnds(jn,:,:,2), cl_grd, 1., pval=999._wp )
      END DO
...
! for NATL60 boxes
      DO ji = 1, 9999
         WRITE(cl4,'(i4.4)') ji
         CALL iom_update_file_name('box'//cl4)
      END DO
